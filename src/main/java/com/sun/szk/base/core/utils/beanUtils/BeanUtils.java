...
public class BeanUtils {
	...
	/**
	 * 类似BeanUtisl.copyProperties复制对象 不同之处可以对同名但不同类型的属性赋值 可以配置类型转换方法 eg：String赋值给Date 需要编写StringToDate方法
	 *
	 * @param target
	 *            目标对象
	 * @param source
	...
	/**
	 * 指定目标对象需要复制的有效属性和无效属性 支持属性间类型转换 配置类型转换方法 eg：String赋值给Date 需要编写StringToDate方法
	 *
	 * @param target
	 *            目标对象
	 * @param source
			...
			for (PropertyDescriptor targetPd : targetPds) {
				...
				if (null != targetPd.getWriteMethod() && null != sourcePd
					...
					if (sourcePd.getPropertyType().isAssignableFrom(targetPd.getPropertyType())) {
						...
						String converMethod = getMethodRuleName(sourcePd.getPropertyType().getName(),
								targetPd.getPropertyType().getName());
	...
	private static void copyCollection(Object target, String[] effectiveProperties, String[] ignoreProperties,
			Boolean nullBeCopied, PropertyDescriptor targetPd, Object sourceValue, Method writeMethod)
					throws IllegalAccessException, InvocationTargetException, UtilException, ClassNotFoundException,
					InstantiationException, NoSuchFieldException {
		Method targetReadMethod = targetPd.getReadMethod();
		@SuppressWarnings("unchecked")
		Collection<Object> targetValue = (Collection<Object>) targetReadMethod.invoke(target);
	...
	public static void setEntryToNull(Object object) {
		...
		for (PropertyDescriptor objectPd : objectPds) {
			if (objectPd.getPropertyType().isAssignableFrom(String.class) && null != objectPd.getWriteMethod()) {
				try {
					String value = (String) objectPd.getReadMethod().invoke(object);
	 ...
	 */
	public static void setPropertyValueByPName(Object targetObject, String property, Object... args)
			throws UtilException {
		try {
